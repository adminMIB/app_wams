...
@push('js')
    ...
    @if (session('error'))
        <script>
            $(document).ready(function() {
                $('#toast-danger').text('{{ session('error') }}');
...
@section('content')
                                    ...
                                    @elseif ($key == 'component')
                                        @if (!empty($value))
                                            @foreach ($value as $item)
                                                <span
                                                    class="badge bg-label-primary mr-2">{{ ucwords(str_replace('_', ' ', $item)) }}</span>
                                            @endforeach
                                        @else
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <button type="button" class="btn btn-md btn-icon me-2" data-bs-toggle="tooltip"
                                            data-bs-placement="top" data-bs-custom-class="tooltip-dark"
                                            data-bs-original-title="Edit Data"
                                            onclick="edit_project_maker({{ $tm->id }})">
                                            <i class="ti ti-edit"></i>
                                        </button>

                                        <button type="button" class="btn btn-md btn-icon me-2 move" data-bs-toggle="tooltip"
                                            data-bs-placement="top"
                                            data-bs-custom-class="tooltip-dark"
                                            data-project="{{ $project['id'] }}"
                                            data-id="{{ $tm->id }}"
                                            data-bs-original-title="Pindah Data">
    ...
    {{-- error toast --}}
    <div class="bs-toast toast toast-ex animate__animated my-2" id="error-toast" role="alert" aria-live="assertive"
        aria-atomic="true" data-bs-delay="10000">
        <div class="toast-header">
