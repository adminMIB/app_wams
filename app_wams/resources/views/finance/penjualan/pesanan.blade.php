...
<section class="section">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <a href="{{ route('pesananpenjualan', $shorder->lto_id) }}" class="btn btn-secondary btn-sm">Back</a>
            <div class="text-end">
                @if ($shorder->status == "Terproses")
                <a href="{{ route('faktur-create', $shorder->id) }}" class="btn btn-primary btn-sm">Create Faktur</a>
                @endif
                <a href="{{route('pesanan-penjualan-pdf', $shorder->id) }}" class="btn btn-danger btn-sm">Print Pdf</a>
                    ...
                    $total = 0;
                        ...
                        {{-- <a href="{{route('penawaran.edit',$i->id)}}">     --}}
                            <tr  style="font-size: 13px;">
                                <td><a href="{{route('pesanan.edit',$i->id)}}">{{$i->nama_barang}}</a></td>
                                <td><a href="{{route('pesanan.edit',$i->id)}}">{{$shorder->kode_project}}</a></td>
                                <td>{{$i->kuantitas}}</td>
                                @if ($i->satuan)
                                <td>{{$i->satuan}}</td>
                                @else
    ...
    <div id="addbrg{{$shorder->id}}" class="modal fade modal-lg" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="card">
                    <div class="card-header">
                        <div class="mb-2 row">
                            ...
                            <label  class="col-sm-1 col-form-label" style="font-size: 10px">Nmber :</label>
                            <div class="col-sm-3">
                ...
                <form action="{{route('add.barang',$shorder->id)}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
                        ...
                        <div class="mb-3 mt-4 row">
                            ...
                            <div class="col-sm-6">
                                <input type="number" class="form-control text-sm " placeholder="Rp." name="total_diskon" id="total" readonly>
                            </div>
                        </div>
