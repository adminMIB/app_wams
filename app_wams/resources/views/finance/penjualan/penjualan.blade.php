...
<section class="section">
    <div class="section-header">
        <div class="card">
            <div class="alert">
                <h2 class="text-capitalize">INVOICE BILLING/SALES</h2>
            </div>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-bordered " id="table1">
                ...
                <tbody>
                    ...
                    <tr align="center" style="font-size:13px">
                        <td><a href="{{route('penawaran-Penjualan',$id->id)}}">{{ $id->kode_project }}</a></td>
                        <td>{{$id->institusi}}</td>
                        <td>{{$id->project}}</td>
                        @if ($id->start_date)
                        ...
                        @endif
                        <td>{{$id->st_project}}</td>
                        {{-- <td><a href="{{route('viewpenjualanD',$id->id)}}" class="btn btn-info btn-sm">Detail</a></td> --}}
                    </tr>
                    @endforeach
                </tbody>
            </table>
