            ...
            <table class="table table-bordered " id="table1">
                ...
                <tbody>
                    ...
                    <tr style="font-size:13px">
                        <td><a onclick="show({{$item->id}})" >{{$item->nama_client}}</a></td>
                        ...
                        <td class="text-center">
                            <div class="btn-group dropstart mb-1" style="cursor: pointer;">
                                ...
                                <div class="dropdown-menu text-center">
                                    ...
                                    @if ($item->status == 'Faktur')
                                    <button type="button" class="btn btn-primary btn-sm"  >{{$item->status}}</button>
                                    @else
                                    <button type="button" class="btn btn-primary btn-sm"  onclick="show({{$item->id}})">Update Status</button>
                                    @endif
...
<div class="modal fade modal-xl" id="syarat" tabindex="-1" aria-labelledby="syaratlabel" aria-hidden="true">
    <div class="modal-dialog">
    <div class="modal-content">
        <div class="modal-header">
        <h5 class="modal-title" id="syaratlabel"><i class="bi bi-exclamation "></i>TERMS</h5>
...
<script>
    function show(id){
        $.get("{{url('edit-status')}}/"+ id,{},function(data,status){
            $("#syaratlabel").html('<i class="bi bi-exclamation">Syarat</i>')
            $("#page").html(data);
            $("#syarat").modal('show');
        });
    }
