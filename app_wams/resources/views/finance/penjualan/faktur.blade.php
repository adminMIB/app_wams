...
<section class="section">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <a href="{{ route('detailfakturpenjualan', $shorder->pesanan->lto_id) }}" class="btn btn-secondary btn-sm">Back</a>
            <div class="text-end">
                <a href="{{route ('faktur-penjualan-pdf', $shorder->id)}}" class="btn btn-danger btn-sm">Cetak Pdf</a>
                <button type="submit" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#addbrg{{$shorder->id}}">Add Barang</button>
            </div>
        </div>
        <div class="card-body">
            <table class="table" id="table1">
                ...
                <tbody>
                    ...
                    $total_ppn = 0;
                    $total = 0;
                        foreach ($shorder->barangfaktur as $i){
                            ?>
                        {{-- <a href="{{route('penawaran.edit',$i->id)}}">     --}}
                            <tr  style="font-size: 13px;">
                                <td><a href="{{route('faktur.edit',$i->id)}}">{{$i->nama_barang}}</a></td>
                                <td><a href="{{route('faktur.edit',$i->id)}}">{{$shorder->pesanan->kode_project}}</a></td>
                                <td>{{$i->kuantitas}}</td>
                                @if ($i->satuan)
                                <td>{{$i->satuan}}</td>
                                @else
            ...
            <div class="row justify-content-end">
                ...
                <div class="col-md-3">
                    <div class="p-1 text-sm" style="border: 1px solid;border-top: 2px solid rgb(255, 0, 0);border-bottom: 2px solid rgb(255, 0, 0);">
                        <label for=""><b>PPN 11%</b></label>
                        <h6 class="text-end">Rp. {{number_format($total_ppn)}}</h6>
                    </div>
                </div>
                <div class="col-md-3">
            ...
            <div class="modal-content">
                <div class="card">
                    <div class="card-header">
                        <div class="mb-2 row">
                            <label  class="col-sm-1 col-form-label" style="font-size: 10px">
                                Order By</label>
                            <div class="col-sm-5">
                            ...
                            <div class="col-sm-3">
                                <input type="text" class="form-control text-sm" name="" value="{{$shorder->no_faktur}}">
                            </div>
                        </div>
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-3">
                                <input type="text" value="{{$shorder->tgl_faktur}}" class="form-control text-sm" name="tgl_penjualan">
                            </div>
                        </div>
                    </div>
                </div> 
                <form action="{{route('add.barangfaktur',$shorder->id)}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <div class="modal-body">
                        ...
                        <div class="mb-2 row">
                            ...
                            <div class="col-sm-10">
                                <input type="number" class="form-control text-sm" value="1" name="kuantitas" id="q" >
                            </div>
                        </div> 
                        ...
                        <div class="mb-3 mt-4 row">
                            ...
                            <div class="col-sm-6">
                                <input type="number" class="form-control text-sm " placeholder="Rp." id="total" name="total_diskon" readonly>
                            </div>
                        </div>
