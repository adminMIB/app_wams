...
<section class="section">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <a href="{{ route('detailFaktur', $shorder->pembelian->lto_id) }}" class="btn btn-secondary btn-sm">Back</a>
            <div class="text-end">
                {{-- <a href="{{route ('faktur-pembelian-pdf', $shorder->id)}}" class="btn btn-danger btn-sm">Cetak Pdf</a> --}}
                <button type="submit" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#addbrg{{$shorder->id}}">Add Stuff</button>
            </div>
        </div>
        <div class="card-body">
            <table class="table" id="table1">
                ...
                <tbody>
                    ...
                    $total_ppn = 0;
                    $total = 0;
                        foreach ($shorder->fakturpembelian as $i){
                            ?>
                        {{-- <a href="{{route('penawaran.edit',$i->id)}}">     --}}
                            <tr  style="font-size: 13px;">
                                <td><a href="{{route('faktur.editP',$i->id)}}">{{$i->nama_barang}}</a></td>
                                <td><a href="{{route('faktur.editP',$i->id)}}">{{$shorder->pembelian->kode_project}}</a></td>
                                <td>{{$i->kuantitas}}</td>
                                @if ($i->satuan)
                                <td>{{$i->satuan}}</td>
                                @else
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-3">
                                <input type="text" value="{{$shorder->tanggal_faktur}}" class="form-control text-sm" name="tgl_penjualan">
                            </div>
                        </div>
                    </div>
                </div> 
                <form action="{{route('add.barangfakturP',$shorder->id)}}" method="POST" enctype="multipart/form-data">
                    {{csrf_field()}}
