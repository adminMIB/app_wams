    ...
    <div class="card" style="border-radius: 2em">
        <div class="card-header">
          ...
          <div class="card-header-action">
            <a href="#" class="addreport btn btn-success">Add Report</a>
                  ...
                  <div class="form-group p-3">
                    ...
                    <select class="selectric-wrapper form-control selectric-selectric selectric-below selectric-open" style="width: 23rem;  margin-bottom:25px;" id="id" name="listp_id">
                      <div class="selectric-hide-select">
                        ...
                        @if (Auth::user()->name == $item->nama_technical || Auth::user()->hasRole('Super Admin'))
                    ...
                    <input type="text" id="jenis_pekerjaan" name="job_essay[]" class="form-control" style="width: 23rem; margin-bottom:25px;" readonly required>
                    {{-- <select class="selectric-wrapper form-control selectric-selectric selectric-below selectric-open" style="width: 23rem;  margin-bottom:25px;" id="job_essay" name="job_essay[]">
                      <div class="selectric-hide-select">
                        <option value="" class="form-control selectric"></option> 
                        @foreach ($listp as $value)
                        @foreach ($value->detail as $item)
                        @if (Auth::user()->name == $item->nama_technical || Auth::user()->hasRole('Super Admin'))
                        <option value="{{ $item->jenis_pekerjaan }}" class="form-control selectric">{{ $item->jenis_pekerjaan }}</option>  
                        @endif
                        @endforeach
                        @endforeach
                      </div>
                      ...
                      {{-- <label for="job_essay">Uraian Pekerjaan</label>
                      <textarea class="form-control" id="jenis_pekerjaan" name="job_essay[]" style="height: 37px; width:23rem;"></textarea>               --}}
                  </div>
                </div>
                <div class="col-6">
                  <div class="form-group p-3">
...
<script type="text/javascript">
  ...
  function addreport() {
    var report = '<div class="row"><div class="col-6"><div class="form-group p-3">  <label for="job_essay">Uraian Pekerjaan</label> <select class="selectric-wrapper form-control selectric-selectric selectric-below selectric-open" style="width: 23rem;  margin-bottom:25px;" id="job_essay" name="job_essay[]">  <div class="selectric-hide-select">  <option value="" class="form-control selectric"></option>  @foreach ($listp as $value) @foreach ($value->detail as $item)  @if (Auth::user()->name == $item->nama_technical || Auth::user()->hasRole('Super Admin')) <option value="{{ $item->jenis_pekerjaan }}" class="form-control selectric">{{ $item->jenis_pekerjaan }}</option> @endif @endforeach @endforeach </div></select></div></div> <div class="col-6"> <div class="form-group p-3"> <div class="form-row">  <div class="form-group col-md-6"> <label for="start_date">Start Date</label> <input type="date" class="form-control" id="start_date" name="start_date[]"> </div> <div class="form-group col-md-6"> <label for="end_date">End Date</label> <input type="date" class="form-control" id="end_date" name="end_date[]"></div></div> <label for="status">Select</label> <select class="form-control" id="status" name="status[]" style="width: 23rem; margin-bottom:25px;"> <option>Pilih Status....</option> <option>Done</option> <option>Issue</option> <option>OnProgress</option> </select> <label for="note">Skripsi</label> <input type="text" class="form-control" id="note" name="note[]" style="width: 23rem; margin-bottom:25px;"> <a  class="remove btn btn-icon btn-danger" href="#" style="float:right;"><i class="fas fa-trash"></i></a> </div></div>  </div></div>   ';
...
<script>
 ...
 $('#id').change(function(){
    ...
    $.ajax({
      ...
      const jns = res.weeklyreport.map((item) => {
        if (user == item.nama_technical) {
          return  item.jenis_pekerjaan;
        }
      });
      // const jns = res.weeklyreport.map(item => item.jenis_pekerjaan)
      $("#nama_institusi").val(res.nama_institusi)
      $("#nama_project").val(res.nama_project)
      $("#jenis_pekerjaan").val(jns)
      $("#listp_id").val(res.id)
    })
 });
</script>
