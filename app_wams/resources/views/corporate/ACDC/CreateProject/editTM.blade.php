<form action="{{route('update-TMACDC',$item->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-2 row">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" name="id_project" id="project_id" value="" readonly required>
        </div>
    </div>
{{--
    <div class="mb-2 row">
        ...
        $.ajax({
            ...
            success: function (r) {
                ...
                for (var i = 0; i < r.length; i++) {
                    if (r.length == 1) {
                        element += '<option value="' + r[i].id_project + '" selected>' + r[i].project_name + '</option>';
                    } else {
                        element += '<option value="' + r[i].id_project + '">' + r[i].project_name + '</option>';
                    }
                }
